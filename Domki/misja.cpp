...
shared_ptr<sfg::Table> interfejs_ranking(MisjaUstawienia &stan, Rozgrywka& rozgrywka, int instance)
{
    ...
    for (auto& gracz : gracze)
    {
        ...
        if (gracz.istotny)
        {
            ...
            GUI::apka().pulpit.SetProperty<sf::Color>("Button#" + graczId, "BackgroundColor", gracz.kolor);
            GUI::apka().pulpit.SetProperty("Button#" + graczId, "FontSize", 26);
            GUI::apka().pulpit.SetProperty("Label#" + graczId, "FontSize", 28);
            GUI::apka().pulpit.SetProperty("Label#" + graczId, "FontName", "Grafika/BuxtonSketch.ttf");
            GUI::apka().pulpit.SetProperty("Label#" + graczProdukcjaId, "Color", gracz.kolor);
            GUI::apka().pulpit.SetProperty("Label#" + graczProdukcjaId, "FontSize", 28);
            GUI::apka().pulpit.SetProperty("Label#" + graczModernizacjaId, "Color", sf::Color::Yellow);
            GUI::apka().pulpit.SetProperty("Label#" + graczModernizacjaId, "FontSize", 28);
            GUI::apka().pulpit.SetProperty("Label#" + graczSzybkoscId, "Color", sf::Color::Black);
            GUI::apka().pulpit.SetProperty("Label#" + graczSzybkoscId, "FontSize", 28);
            GUI::apka().pulpit.SetProperty("Label#" + graczLudkiId, "FontName", "Grafika/waltographUI.ttf");
            GUI::apka().pulpit.SetProperty("Label#" + graczLudkiId, "FontSize", 28);
            if (stan.Zwyciezca() == nr)
            {
                GUI::apka().pulpit.SetProperty("Button#" + graczId, "FontSize", 80.f);
                GUI::apka().pulpit.SetProperty("Label#" + graczId, "FontSize", 80.f);
            }
...
shared_ptr<sfg::Window> interfejs_rozgrywki(shared_ptr<sfg::Window> interfejs, MisjaUstawienia &stan, Rozgrywka& rozgrywka, Wyswietlacz& wyswietlacz, Domek* wybrany)
{
    ...
    if (interfejs == nullptr)
    {
        if (stan.do_ilu_wygranych > 0)
        {
            ...
            GUI::apka().pulpit.Add(interfejs);
            interfejs->Add(tabela_interfejsu);
            GUI::apka().bottom_left_window(interfejs);
        }
    }
    else
    {
...
void odliczanie(Wyswietlacz& wyswietlacz, sf::View widok, std::shared_ptr<sfg::Window> gui_pasek)
{
    ...
    GUI::apka().show_bottom_gui(widok, gui_pasek);
    for (int a = 3; a >= 0; a--)
    {
        GUI::apka().okno.clear();

        wyswietlacz.WyswietlTlo(GUI::apka().okno);
        if (a>0) odliczanie_etykieta->SetText(std::to_string(a));
        else odliczanie_etykieta->SetText("RUSZAJ!");
        wyswietlacz.Wyswietlaj(GUI::apka().okno);

        GUI::apka().show_bottom_gui(widok, gui_pasek);
        GUI::apka().okno.display();

        if (a > 0)
        {
            if (GUI::apka().dzwieki_glosnosc)
                pikPik.play();
            Sleep(1500);
        }
        else
...
void zakonczenie_gry(Gracz& gracz_wygrany, int grajacy)
{
    ...
    GUI::apka().show_and_wait_for_anything(okno);
}
...
sf::View wysrodkowany_widok(list<Domek> &domki, int bottom_space)
{
    ...
    auto okno_size = GUI::apka().okno.getSize();
...
int misja(MisjaUstawienia& misja_ustawienia, Ruszacz& ruszacz)
{
    ...
    sf::RenderWindow& window = GUI::apka().okno;

    if (misja_ustawienia.nr_gracza == 0)
    {
        window.setVisible(false);
        GUI::apka().dzwieki_glosnosc = 0;
    }
    ...
    while (trwa_gra)
    {
        ...
        {
            GUI::apka().pulpit.HandleEvent(event);
        ...
        GUI::apka().show_bottom_gui(view, interfejs);
        ...
        if (rozgrywka.liczba_aktywnych_graczy == 1)
        {
            ...
            GUI::apka().finish_viewport_render(view);
            if (interfejs != nullptr)
                GUI::apka().remove_active_window(interfejs);
