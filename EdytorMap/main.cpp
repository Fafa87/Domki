...
int pokazowa_misja()
{
    ...
    while (window.isOpen())
    {
        ...
        if (ruchy.size())
        {
            if (IsType<WymarszRozkaz>(ruchy[0]))
            {
                auto& ruch = *(WymarszRozkaz*)ruchy[0];
                ruch.skad->drogi.push_back(ruch.dokad);

                ruch.skad->polozenie.x = (ruch.skad->polozenie.x + ruch.dokad->polozenie.x) / 2.0;
                ruch.skad->polozenie.y = (ruch.skad->polozenie.y + ruch.dokad->polozenie.y) / 2.0;
            }
        }
        ...
        interfejs_rozgrywki(gujak, ustawienia, rozgrywka, wyswietlacz, ruchGracza.WybranyDomek(), myszkaGracza.skupiony);
